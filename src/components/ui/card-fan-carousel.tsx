...
export interface CardItem {
  ...
  alt?: string;
  linkUrl?: string;
  // Extended properties for rich content (e.g. certificates, courses)
  title?: string;
  issuer?: string;
  date?: string;
  badge?: string;
...
export default function SocialCards({ cards, className = "", onCardClick }: SocialCardsProps) {
  ...
  return (
    <section className={`fan-carousel-section flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20 ${className}`}>
          ...
          {cards.map((card, index) => {
            ...
            const image = (
              <div className="fan-card-inner relative w-full h-full overflow-hidden rounded-2xl">
                ...
                {hasDetails && (
                  <>
                    <div className="fan-card-overlay absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent z-20 pointer-events-none" />
                    <div className="fan-card-content absolute inset-x-0 bottom-0 p-4 z-30 flex flex-col justify-end text-left pointer-events-none">
                      {card.date && (
                        <div className="flex items-center justify-between mb-2">
                          <span className="fan-card-badge inline-block text-[0.62rem] font-mono uppercase tracking-widest text-[#e2c685] bg-black/60 backdrop-blur-md border border-[#c8a45a]/30 px-2.5 py-0.5 rounded-full">
                            {card.date}
                          </span>
                          {card.hasLiveCert && (
                            <span className="fan-card-live text-[0.58rem] font-mono font-semibold text-[#f2ede3] bg-[#c8a45a]/30 border border-[#c8a45a]/50 px-2 py-0.5 rounded-full flex items-center gap-1">
                              Live ↗
                            </span>
                          )}
                        </div>
                      ...
                      {card.title && (
                        <h3 className="fan-card-title text-sm md:text-base font-bold text-white leading-tight drop-shadow-md line-clamp-2">
                          {card.title}
                        </h3>
                      )}
                      {card.issuer && (
                        <p className="fan-card-issuer text-xs text-zinc-300 mt-1 line-clamp-1">
                          {card.issuer}
                        </p>
                      )}
                    </div>
            ...
            if (card.onClick || onCardClick) {
              return (
                <button
                  type="button"
                  key={index}
                  className="fan-card block cursor-pointer text-left bg-transparent border-none p-0 outline-none"
                  onClick={handleClick}
                >
                  {image}
                </button>
      ...
      {needsPagination && (
        <div className="fan-carousel-controls flex items-center justify-center gap-4 mt-4 md:mt-6 z-30">
          <button className={`${ARROW_CLASSES} w-10 h-10 md:w-12 md:h-12`} onClick={() => cycle("left")} aria-label="Previous">
            {chevron("left")}
          </button>
          <div className="fan-carousel-dots flex items-center gap-2">
            {cards.map((_, i) => (
              <span
                key={i}
                className={`fan-carousel-dot w-2 h-2 rounded-full transition-all duration-300 ${
                  i === centerIndex
                    ? "fan-dot-active bg-black/70 dark:bg-white/80 scale-[1.3]"
